...
public class WebDriverManager implements Config {
	...
	private AccountPage getAccountPage(AccountDTO accountDto) {
		//
	...
	private Double getDifference(AccountDTO accountDto, Double amount, List<TotalDTO> prevTotals) {
		if (amount == null)
			return 0.00;

		Double prevTotal = prevTotals.stream().filter(t -> t.getAccount() == accountDto).findFirst().get().getAmount();
		if (prevTotal == null)
			return null;
		else
			return NumUtils.roundDouble(amount - prevTotal);

	}

	private void addTransactionsForDifference(TotalDTO totalDto, AccountPage accountPage, Double difference, Double prevTotal,
		...
		try {
			...
			if (totalDto.getTransactions().isEmpty()) {
				...
				//totalDto.setErrorStatus(prevTotal, DataRetrievalStatus.NO_MATCH_FOR_TOTAL, null); //TODO
			} else {
	...
	public void refreshTotals(List<TotalDTO> totalsToRefresh,
			List<TransactionDTO> prevTransactions) {
...
//			executor.submit(() -> {
//				totalsToRefresh.stream().filter(a -> a.getAccount().getBank()==driver).forEach(total -> {
			for(TotalDTO total: totalsToRefresh) {
					...
					while (!isDownloaded && attempt < maxAttemptsToDownloadData) {
						...
						difference = getDifference(accountDto, amount, totalsToRefresh);
...
//		}
	...
	public List<TotalDTO> getNewTotals(List<TotalDTO> prevTotals,
		...
		// Group accounts list by banks
		prevTotals.stream().filter(a -> a.getAccount().getBank() != null).forEach(a -> {
			List<String> driver = new ArrayList<String>();
			driver.add(a.getAccount().getBank().getName());
			if (!drivers.contains(driver))
				drivers.add(driver);
		...
		for (List<String> driver : drivers) {
			executor.submit(() -> {
				prevTotals.stream().filter(a -> a.getAccount().getBank()==driver).forEach(total -> {
					AccountDTO accountDto = total.getAccount();
					Thread.currentThread().setName("Bank accounts ("
							+ SeleniumUtils.getThreadNumber(Thread.currentThread().getName()) + "): " + accountDto.getName());
					int attempt = 0;
					Double amount = null;
					TotalDTO total = null;
					Double prevTotal = total.getAmount();
					Double difference = null;
					boolean isDownloaded = false;
					while (!isDownloaded && attempt < maxAttemptsToDownloadData) {
						logger.info("{}: attempt #{}", accountDto.getName(), ++attempt);
